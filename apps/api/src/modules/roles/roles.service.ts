...
export class RolesService {
    ...
    async remove(tenantId: string, roleId: string) {
        ...
        if (role.isDefault) throw new BadRequestException('No puedes eliminar un rol del sistema');
        if ((role._count as any).users > 0) throw new BadRequestException('No puedes eliminar un rol que tiene usuarios asignados');
    ...
    async seedDefaultRoles(tenantId: string) {
        const defaults = [
            ...
            },
            {
                ...
                displayName: 'Gerente',
                color: '#F0A030',
                permissions: SYSTEM_PERMISSIONS.filter(p => !p.key.includes('settings') && !p.key.includes('billing') && !p.key.includes('users.manage')).map(p => p.key),
                isDefault: true,
            },
            {
                name: 'cashier',
