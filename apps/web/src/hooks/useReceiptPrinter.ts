...
export function buildReceiptHTML(data: ReceiptData, config: PrintConfig): string {
  ...
  if (data.paymentMethod === 'mixed' && data.mixedPayments) {
    paymentHTML = `
      <div class="totals-row"><span>Método</span><span>${PAYMENT_ICONS['mixed']} Mixto</span></div>
    ...
    paymentHTML = `<div class="totals-row"><span>Método de pago</span><span>${PAYMENT_ICONS[data.paymentMethod] ?? ''} ${PAYMENT_LABELS[data.paymentMethod] ?? data.paymentMethod}</span></div>`;
  }
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  @page { width:${W}; margin:0; }
  body {
    font-family: 'Courier New', Courier, monospace;
    font-size: 11px; width:${W}; max-width:${W};
    margin:0 auto; padding:4mm 3mm;
    background:#fff; color:#000;
    -webkit-print-color-adjust:exact; print-color-adjust:exact;
  }
  .divider     { border-top:1px dashed #999; margin:3mm 0; }
  .divider-dbl { border-top:2px solid #000; margin:3mm 0; }
  .business-name  { font-size:16px; font-weight:bold; text-align:center; margin-bottom:2mm; letter-spacing:.5px; }
  .business-sub   { font-size:10px; text-align:center; color:#444; margin-bottom:1mm; }
  .header-custom  { font-size:10px; text-align:center; color:#333; margin-bottom:2mm; font-style:italic; }
  .receipt-title  { font-size:12px; font-weight:bold; text-align:center; letter-spacing:2px; margin:2mm 0; }
  .meta-row       { display:flex; justify-content:space-between; font-size:10px; color:#333; margin-bottom:1mm; }
  .item           { margin-bottom:2mm; }
  .item-name      { font-weight:bold; font-size:11px; word-break:break-word; }
  .item-row       { display:flex; justify-content:space-between; font-size:10px; color:#333; }
  .item-qty       { color:#555; }
  .item-price     { font-weight:bold; color:#000; }
  .item-discount  { font-size:9px; color:#888; }
  .totals-row     { display:flex; justify-content:space-between; font-size:11px; margin-bottom:1.5mm; }
  .totals-row.sub { font-size:10px; color:#555; padding-left:4mm; }
  .totals-row.total-final { font-size:16px; font-weight:bold; margin-top:2mm; }
...
<body>
  ...
  ${line}
  <div class="meta-row"><span>Fecha</span><span>${formatDate(data.createdAt)}</span></div>
  <div class="meta-row"><span>Cajero</span><span>${data.cashierName}</span></div>
  <div class="meta-row"><span>N° Recibo</span><span class="bold">#${data.saleId.slice(-8).toUpperCase()}</span></div>
  ${dblLine}
  ${itemsHTML}
  ${line}
  <div class="totals-row"><span>Subtotal</span><span>${formatCurrency(data.subtotal)}</span></div>
  ${data.discount > 0 ? `<div class="totals-row discount"><span>Descuento</span><span>-${formatCurrency(data.discount)}</span></div>` : ''}
  ${data.tax > 0 ? `<div class="totals-row"><span>Impuesto</span><span>${formatCurrency(data.tax)}</span></div>` : ''}
  ${dblLine}
  <div class="totals-row total-final"><span>TOTAL</span><span>${formatCurrency(data.total)}</span></div>
  ${line}
  ${paymentHTML}
...
export function useReceiptPrinter() {
  ...
  const print = useCallback((data: ReceiptData, config: PrintConfig = DEFAULT_PRINT_CONFIG) => {
    ...
    const win = window.open(url, '_blank', 'width=400,height=600,toolbar=0,menubar=0,scrollbars=1');
    if (!win) {
