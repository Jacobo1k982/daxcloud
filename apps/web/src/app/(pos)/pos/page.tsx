...
export default function POSPage() {
  ...
  const saleMutation = useMutation({
    mutationFn: async () => {
      ...
      if (industry === 'restaurant') {
        const { data } = await api.post('/restaurant/orders', {
          ...
          items: cart.map(item => ({
            ...
            notes: item.notes,
            modifiers: item.modifiers?.map(m => m.optionId),
          })),
        });
        return data;
      }

      // Peluquería → cita (se crea adicionalmente)
      ...
      // ✅ FIX: destructura { data } — antes se retornaba el objeto Axios completo
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['pos-products'] }); // refresca stock visible
      queryClient.invalidateQueries({ queryKey: ['restaurant-tables'] });
    },

    // ✅ FIX: antes los errores fallaban en silencio
    onError: (error: any) => {
      const msg =
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--dax-bg)', fontFamily: 'var(--font-outfit, sans-serif)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div style={{ height: '52px', padding: '0 16px', borderBottom: '1px solid var(--dax-border)', background: 'var(--dax-surface)', display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '30px', height: '30px', borderRadius: '8px', border: '1px solid var(--dax-border)', background: 'var(--dax-surface-2)', cursor: 'pointer', color: 'var(--dax-text-muted)', flexShrink: 0 }}
          ...
          {industry === 'restaurant' && (
            <select
              ...
              style={{ marginLeft: '8px', padding: '5px 10px', borderRadius: '8px', border: `1px solid ${selectedTable ? C : 'var(--dax-border)'}`, background: 'var(--dax-surface-2)', color: selectedTable ? C : 'var(--dax-text-muted)', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}
            >
              <option value="">🪑 Para llevar</option>
              {(tables as any[]).filter((t: any) => t.status === 'available').map((t: any) =>
          ...
          {industry === 'salon' && (
            <select
              ...
              style={{ marginLeft: '8px', padding: '5px 10px', borderRadius: '8px', border: `1px solid ${selectedEmployee ? C : 'var(--dax-border)'}`, background: 'var(--dax-surface-2)', color: selectedEmployee ? C : 'var(--dax-text-muted)', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}
            >
          ...
          <button
            ...
            className="pos-cart-btn-mobile"
            style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', borderRadius: '10px', background: C, color: '#fff', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 700 }}
          >
        ...
        <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--dax-border)', background: 'var(--dax-surface)', display: 'flex', gap: '10px', alignItems: 'center', flexShrink: 0 }}>
          <div style={{ position: 'relative', flex: 1 }}>
            ...
            <input
              ref={searchRef}
              className="dax-input"
              style={{ paddingLeft: '30px', margin: 0, height: '34px', fontSize: '12px' }}
          ...
          <div style={{ display: 'flex', gap: '5px', overflowX: 'auto', flexShrink: 0, maxWidth: '55%' }}>
            <button
              ...
              style={{ padding: '4px 10px', borderRadius: '8px', fontSize: '11px', fontWeight: 600, border: 'none', cursor: 'pointer', background: !selectedCategory ? C : 'var(--dax-surface-2)', color: !selectedCategory ? '#fff' : 'var(--dax-text-muted)', flexShrink: 0, whiteSpace: 'nowrap' }}
            >
              Todo
            </button>
            {(categories as string[]).map(cat => (
              <button
                ...
                style={{ padding: '4px 10px', borderRadius: '8px', fontSize: '11px', fontWeight: 600, border: 'none', cursor: 'pointer', background: selectedCategory === cat ? C : 'var(--dax-surface-2)', color: selectedCategory === cat ? '#fff' : 'var(--dax-text-muted)', flexShrink: 0, whiteSpace: 'nowrap' }}
              >
                {cat}
              </button>
            ))}
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '8px' }}>
                {(services as any[]).map((s: any) => (
                  <button
                    key={s.id}
                    onClick={() => addToCart({ id: s.id, name: s.name, price: Number(s.price) }, { serviceId: s.id, employeeId: selectedEmployee })}
                    style={{ padding: '10px', background: 'var(--dax-surface)', border: `1px solid ${s.color ?? C}30`, borderRadius: '10px', cursor: 'pointer', textAlign: 'left' }}
                  >
              ...
              {(products as any[]).length > 0 && (
                <p style={{ fontSize: '10px', fontWeight: 700, color: 'var(--dax-text-muted)', letterSpacing: '.1em', textTransform: 'uppercase', marginTop: '16px', marginBottom: '8px' }}>Productos</p>
              )}
            </div>
          )}
                  ...
                  <div key={pp.id} style={{ padding: '10px', background: 'var(--dax-surface)', border: '1px solid var(--dax-border)', borderRadius: '10px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '5px' }}>
                      <input
                        ...
                        style={{ flex: 1, padding: '5px 7px', borderRadius: '7px', border: '1px solid var(--dax-border)', background: 'var(--dax-surface-2)', color: 'var(--dax-text-primary)', fontSize: '11px', minWidth: 0 }}
                      />
                      <button
                        onClick={() => {
                          const w = parseFloat(weightInput[pp.id] ?? '0');
                          if (w > 0) {
                            addToCart({ id: pp.productId, name: pp.product?.name, price: Number(pp.pricePerUnit) }, { quantity: w, unit: pp.weightUnit, weight: w });
                            setWeightInput(p => ({ ...p, [pp.id]: '' }));
                          }
                        }}
                        style={{ padding: '5px 8px', background: C, color: '#fff', border: 'none', borderRadius: '7px', fontSize: '13px', fontWeight: 800, cursor: 'pointer' }}
                      >+</button>
                    </div>
                  </div>
                ))}
              ...
              {filtered.map((product: any) => {
                ...
                return (
                  <button
                    key={product.id}
                    onClick={() => {
                      if (industry === 'clothing' && pvariants.length > 0) { setShowVariantModal({ product, variants: pvariants }); return; }
                      if (industry === 'restaurant') { setShowModifierModal(product); return; }
                      addToCart(product);
                    }}
                    style={{ padding: '10px', background: inCart ? `${C}08` : 'var(--dax-surface)', border: `1.5px solid ${inCart ? C : 'var(--dax-border)'}`, borderRadius: '10px', cursor: 'pointer', textAlign: 'left', transition: 'all .12s', position: 'relative', boxShadow: inCart ? `0 0 0 1px ${C}30` : 'none' }}
                  >
                    {product.imageUrl && (
                      <img src={product.imageUrl} alt="" style={{ width: '100%', height: '60px', objectFit: 'cover', borderRadius: '7px', marginBottom: '7px' }} />
                    )}
                    {inCart && (
                      <div style={{ position: 'absolute', top: '7px', right: '7px', background: C, color: '#fff', borderRadius: '6px', width: '18px', height: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: 800 }}>
                        {inCart.quantity}
                      </div>
                    )}
                    {hasOffer && (
                      <div style={{ position: 'absolute', top: '7px', left: '7px', background: '#F97316', color: '#fff', borderRadius: '5px', padding: '1px 5px', fontSize: '8px', fontWeight: 800 }}>⚡</div>
                    )}
                    <p style={{ fontSize: '11px', fontWeight: 600, color: 'var(--dax-text-primary)', marginBottom: '3px', lineHeight: 1.3 }}>{product.name}</p>
                    {product.sku && (
                      <p style={{ fontSize: '9px', color: 'var(--dax-text-muted)', marginBottom: '3px', fontFamily: 'monospace' }}>{product.sku}</p>
                    )}
                    {hasOffer && (
                      <p style={{ fontSize: '9px', color: 'var(--dax-text-muted)', textDecoration: 'line-through' }}>{formatCurrency(Number(product.price))}</p>
                    )}
                    <p style={{ fontSize: '13px', fontWeight: 800, color: hasOffer ? '#F97316' : C, lineHeight: 1 }}>{formatCurrency(price)}</p>
                    {industry === 'clothing' && pvariants.length > 0 && (
                      <p style={{ fontSize: '9px', color: C, marginTop: '3px', fontWeight: 600 }}>{pvariants.length} variantes</p>
                    )}
                  </button>
                );
              })}
      ...
      <div
        ...
        style={{ width: '300px', borderLeft: '1px solid var(--dax-border)', display: 'flex', flexDirection: 'column', background: 'var(--dax-surface)', flexShrink: 0 }}
      >
        <CartSide
      ...
      {showCart && (
        ...
        >
          <div
            style={{ width: '100%', maxHeight: '85vh', background: 'var(--dax-surface)', borderRadius: '16px 16px 0 0', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
          ...
          >
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--dax-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              ...
              <button onClick={() => setShowCart(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--dax-text-muted)', display: 'flex' }}>
                <X size={18} />
              </button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto' }}>
              <CartSide
                ...
                paymentMethod={paymentMethod} setPaymentMethod={setPaymentMethod}
                updateQty={updateQty} removeFromCart={removeFromCart}
                onSell={() => { saleMutation.mutate(); setShowCart(false); }}
                isSelling={saleMutation.isPending}
                C={C} fmt={formatCurrency} industry={industry} selectedTable={selectedTable}
              />
            </div>
      ...
      {showVariantModal && (
        <VariantModal
          data={showVariantModal}
          setData={setShowVariantModal}
          C={C} fmt={formatCurrency} addToCart={addToCart}
        />
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'var(--dax-surface)', borderRadius: '14px', padding: '22px', width: '100%', maxWidth: '420px', maxHeight: '80vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <button
                onClick={() => { setShowModifierModal(null); setSelectedModifiers([]); }}
              ...
              : (modifierGroups as any[]).map((group: any) => (
                <div key={group.id} style={{ marginBottom: '14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
                    ...
                    {group.required && (
                      <span style={{ fontSize: '9px', background: 'var(--dax-danger-bg)', color: 'var(--dax-danger)', padding: '2px 5px', borderRadius: '4px', fontWeight: 700 }}>Requerido</span>
                    )}
                  </div>
                    ...
                    {group.options?.map((opt: any) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            if (sel) {
                              setSelectedModifiers(p => p.filter(m => m.optionId !== opt.id));
                            } else {
                              if (!group.multiple)
                                setSelectedModifiers(p => p.filter(m => !group.options.find((o: any) => o.id === m.optionId)));
                              setSelectedModifiers(p => [...p, { optionId: opt.id, name: opt.name, extraPrice: Number(opt.extraPrice) }]);
                            }
                          }}
                          style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 12px', background: sel ? `${C}10` : 'var(--dax-surface-2)', border: `1px solid ${sel ? C : 'transparent'}`, borderRadius: '8px', cursor: 'pointer', textAlign: 'left' }}
                        >
            ...
            <button
              onClick={() => {
                addToCart(showModifierModal, { modifiers: selectedModifiers });
                setShowModifierModal(null);
                setSelectedModifiers([]);
              }}
              style={{ width: '100%', padding: '12px', background: C, color: '#fff', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: 800, cursor: 'pointer', marginTop: '6px' }}
            >
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--dax-border)', flexShrink: 0 }}>
        ...
        {industry === 'restaurant' && selectedTable && (
          <p style={{ fontSize: '10px', color: C, marginTop: '2px', fontWeight: 600 }}>🍽️ Mesa asignada</p>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 12px' }}>
        {cart.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '8px', color: 'var(--dax-text-muted)' }}>
            <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: `${C}10`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <ShoppingCart size={20} color={C} style={{ opacity: .4 }} />
            </div>
          ...
          <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 0', borderBottom: '1px solid var(--dax-border-soft)' }}>
            ...
            <div style={{ width: '24px', height: '24px', borderRadius: '6px', background: `${C}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <span style={{ fontSize: '11px', fontWeight: 800, color: C }}>{item.quantity}</span>
            </div>
            {/* Info */}
      ...
      {cart.length > 0 && (
        <div style={{ padding: '10px 12px', borderTop: '1px solid var(--dax-border)', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: '3px' }}>
              {[0, 5, 10, 15, 20].map(p => (
                <button
                  ...
                  style={{ padding: '2px 6px', borderRadius: '6px', fontSize: '9px', fontWeight: 700, border: 'none', cursor: 'pointer', background: discount === p ? C : 'var(--dax-surface-2)', color: discount === p ? '#fff' : 'var(--dax-text-muted)' }}
                >
                  {p === 0 ? '—' : `${p}%`}
                </button>
              ))}
          ...
          <input
            ...
            placeholder="Nota..."
            style={{ width: '100%', padding: '6px 9px', borderRadius: '8px', border: '1px solid var(--dax-border)', background: 'var(--dax-surface-2)', color: 'var(--dax-text-primary)', fontSize: '11px', marginBottom: '8px', boxSizing: 'border-box' }}
          />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px', marginBottom: '8px' }}>
            {PAYMENT_METHODS.map(m => (
              <button
                ...
                style={{ padding: '7px 6px', borderRadius: '8px', fontSize: '10px', fontWeight: 600, border: `1.5px solid ${paymentMethod === m.value ? C : 'var(--dax-border)'}`, background: paymentMethod === m.value ? `${C}10` : 'transparent', color: paymentMethod === m.value ? C : 'var(--dax-text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}
              >
                {m.icon} {m.label}
              </button>
            ))}
          ...
          <button
            onClick={onSell}
            disabled={isSelling}
            style={{ width: '100%', padding: '12px', background: isSelling ? 'var(--dax-surface-2)' : C, color: isSelling ? 'var(--dax-text-muted)' : '#fff', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: 800, cursor: isSelling ? 'not-allowed' : 'pointer', boxShadow: isSelling ? 'none' : `0 3px 12px ${C}35`, transition: 'all .15s' }}
          >
            {isSelling
              ? '⏳ Procesando...'
              : industry === 'restaurant' && selectedTable
...
function VariantModal({ data, setData, C, fmt, addToCart }: any) {
  ...
  const match = data.variants.find((v: any) =>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: 'var(--dax-surface)', borderRadius: '14px', padding: '22px', width: '100%', maxWidth: '420px', maxHeight: '80vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <button onClick={() => setData(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--dax-text-muted)', display: 'flex' }}>
            <X size={18} />
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                {sizes.map(s => (
                  <button
                    key={s}
                    onClick={() => setSelSize(s === selSize ? '' : s)}
                    style={{ padding: '7px 14px', borderRadius: '8px', fontSize: '12px', fontWeight: 700, border: `2px solid ${selSize === s ? C : 'var(--dax-border)'}`, background: selSize === s ? `${C}12` : 'var(--dax-surface-2)', color: selSize === s ? C : 'var(--dax-text-secondary)', cursor: 'pointer' }}
                  >{s}</button>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {data.variants
                .filter((v: any, i: number, a: any[]) => a.findIndex((x: any) => x.color === v.color) === i)
                .map((v: any) => (
                  <button
                    key={v.color}
                    onClick={() => setSelColor(v.color === selColor ? '' : v.color)}
                    style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '5px 10px', borderRadius: '8px', border: `2px solid ${selColor === v.color ? C : 'var(--dax-border)'}`, background: selColor === v.color ? `${C}12` : 'var(--dax-surface-2)', cursor: 'pointer' }}
                  >
          ...
          {match && (
            <div style={{ background: 'var(--dax-surface-2)', borderRadius: '8px', padding: '8px 10px', display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '11px', color: 'var(--dax-text-muted)' }}>
                Stock: <strong style={{ color: match.stock > 0 ? 'var(--dax-success)' : 'var(--dax-danger)' }}>{match.stock}</strong>
              </span>
          ...
          <button
            onClick={() => {
              ...
              addToCart(data.product, { variantId: match.id, size: selSize, color: selColor, price: match.price ? Number(match.price) : undefined });
              setData(null);
            }}
            disabled={!match || match.stock === 0}
            style={{ width: '100%', padding: '11px', background: !match || match.stock === 0 ? 'var(--dax-surface-2)' : C, color: !match || match.stock === 0 ? 'var(--dax-text-muted)' : '#fff', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: 800, cursor: !match || match.stock === 0 ? 'not-allowed' : 'pointer' }}
          >
