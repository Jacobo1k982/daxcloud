...
export default function POSPage() {
  ...
  const getPrice = (product: any) => {
    ...
    const hh = (activeHappyHour as any[]).find((h: any) => h.products?.some((p: any) => p.productId === product.id));
  ...
  const addToCart = (product: any, options?: any) => {
    ...
    setCart(prev => {
      ...
      if (ex && !options?.modifiers?.length) return prev.map(i => i.id === itemId ? { ...i, quantity: i.quantity + qty } : i);
  ...
  const updateQty = (id: string, delta: number) => setCart(prev =>
    prev.map(i => i.id !== id ? i : i.quantity + delta <= 0 ? null : { ...i, quantity: i.quantity + delta }).filter(Boolean) as CartItem[]
  );
    ...
    mutationFn: async () => {
      if (industry === 'restaurant') {
        const { data } = await api.post('/restaurant/orders', {
          ...
          notes: note,
          items: cart.map(item => ({ productId: item.productId, quantity: item.quantity, notes: item.notes, modifiers: item.modifiers?.map(m => m.optionId) })),
        });
        return data;
      }
      if (industry === 'salon' && cart[0]?.serviceId) {
        await api.post('/salon/appointments', { clientName: 'Cliente walk-in', employeeId: selectedEmployee || undefined, serviceId: cart[0].serviceId, startTime: new Date().toISOString(), branchId: (branches as any[])[0]?.id });
      }
      return api.post('/sales', { branchId: (branches as any[])[0]?.id, paymentMethod, discount: discountAmt, tax: 0, subtotal, total, notes: note || undefined, items: cart.map(item => ({ productId: item.productId, quantity: item.quantity, unitPrice: item.price, subtotal: item.price * item.quantity, discount: 0, notes: item.notes, variantId: item.variantId })) });
    },
    onSuccess: () => { setCart([]); setDiscount(0); setNote(''); setSelectedTable(''); setSelectedEmployee(''); setShowCart(false); queryClient.invalidateQueries({ queryKey: ['sales'] }); queryClient.invalidateQueries({ queryKey: ['restaurant-tables'] }); },
  });
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--dax-bg)', fontFamily: 'var(--font-outfit, sans-serif)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div style={{ height: '52px', padding: '0 16px', borderBottom: '1px solid var(--dax-border)', background: 'var(--dax-surface)', display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
          ...
          {/* Ícono + nombre industria — ya existente */}
          ...
          {/* Búsqueda + Categorías en una fila */}
          <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--dax-border)', background: 'var(--dax-surface)', display: 'flex', gap: '10px', alignItems: 'center', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: '5px', overflowX: 'auto', flexShrink: 0, maxWidth: '55%' }}>
              <button onClick={() => setSelectedCategory('')} style={{ padding: '4px 10px', borderRadius: '8px', fontSize: '11px', fontWeight: 600, border: 'none', cursor: 'pointer', background: !selectedCategory ? C : 'var(--dax-surface-2)', color: !selectedCategory ? '#fff' : 'var(--dax-text-muted)', flexShrink: 0, whiteSpace: 'nowrap' }}>
                Todo
              </button>
              {(categories as string[]).map(cat => (
                <button key={cat} onClick={() => setSelectedCategory(cat === selectedCategory ? '' : cat)} style={{ padding: '4px 10px', borderRadius: '8px', fontSize: '11px', fontWeight: 600, border: 'none', cursor: 'pointer', background: selectedCategory === cat ? C : 'var(--dax-surface-2)', color: selectedCategory === cat ? '#fff' : 'var(--dax-text-muted)', flexShrink: 0, whiteSpace: 'nowrap' }}>
                  {cat}
                </button>
              ))}
            </div>
        ...
        {/* ══ MODAL VARIANTES ══ */}
        {showVariantModal && (
          <VariantModal data={showVariantModal} setData={setShowVariantModal} C={C} fmt={formatCurrency} addToCart={addToCart} />
        )}
                      ...
                      {group.options?.map((opt: any) => {
                        ...
                        return (
                          <button key={opt.id} onClick={() => {
                            if (sel) { setSelectedModifiers(p => p.filter(m => m.optionId !== opt.id)); }
                            else {
                              if (!group.multiple) setSelectedModifiers(p => p.filter(m => !group.options.find((o: any) => o.id === m.optionId)));
