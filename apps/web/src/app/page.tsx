...
function ParticleField() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      particles = Array.from({ length: Math.floor(W * H / 10000) }, () => ({ x: Math.random() * W, y: Math.random() * H, vx: (Math.random() - .5) * .18, vy: (Math.random() - .5) * .13, r: .3 + Math.random() * 1.0, o: .06 + Math.random() * .2, coral: Math.random() < .2 }));
    };
    const animate = () => {
      animId = requestAnimationFrame(animate); t += .003; ctx.clearRect(0, 0, W, H);
      const g1 = ctx.createRadialGradient(W * .2, H * .3, 0, W * .2, H * .3, W * .4); g1.addColorStop(0, `rgba(255,92,53,${.04 + .015 * Math.sin(t)})`); g1.addColorStop(1, 'transparent'); ctx.fillStyle = g1; ctx.fillRect(0, 0, W, H);
      const g2 = ctx.createRadialGradient(W * .8, H * .6, 0, W * .8, H * .6, W * .35); g2.addColorStop(0, `rgba(30,58,95,${.10 + .03 * Math.cos(t * .7)})`); g2.addColorStop(1, 'transparent'); ctx.fillStyle = g2; ctx.fillRect(0, 0, W, H);
      particles.forEach(p => { p.x += p.vx; p.y += p.vy; if (p.x < 0) p.x = W; if (p.x > W) p.x = 0; if (p.y < 0) p.y = H; if (p.y > H) p.y = 0; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fillStyle = p.coral ? `rgba(255,92,53,${p.o})` : `rgba(91,170,240,${p.o})`; ctx.fill(); });
      for (let i = 0; i < particles.length; i++) for (let j = i + 1; j < particles.length; j++) { const dx = particles[i].x - particles[j].x, dy = particles[i].y - particles[j].y, d = Math.sqrt(dx * dx + dy * dy); if (d < 80) { ctx.beginPath(); ctx.moveTo(particles[i].x, particles[i].y); ctx.lineTo(particles[j].x, particles[j].y); ctx.strokeStyle = `rgba(30,58,95,${.12 * (1 - d / 80)})`; ctx.lineWidth = .5; ctx.stroke(); } }
    };
...
function useReveal(threshold = 0.12) {
  ...
  useEffect(() => { const el = ref.current; if (!el) return; const obs = new IntersectionObserver(([e]) => { if (e.isIntersecting) setVisible(true) }, { threshold }); obs.observe(el); return () => obs.disconnect(); }, []);
  return { ref, visible };
}
...
const FEATURES = [
  ...
  { icon: Package, color: '#EC4899', title: 'Inventario inteligente', desc: 'Control de stock, movimientos y alertas automáticas cuando el inventario baja del mínimo.' },
];

const INDUSTRIES = [
  { icon: Utensils, label: 'Restaurante', color: '#F97316' }, { icon: ChefHat, label: 'Panadería', color: '#FF5C35' },
  { icon: Pill, label: 'Farmacia', color: '#5AAAF0' }, { icon: Scissors, label: 'Peluquería', color: '#A78BFA' },
  { icon: Shirt, label: 'Ropa', color: '#EAB308' }, { icon: Leaf, label: 'Verdulería', color: '#22C55E' },
  { icon: ShoppingCart, label: 'Supermercado', color: '#5AAAF0' }, { icon: Package, label: 'Tienda', color: '#FF5C35' },
];
...
function FeatureCard({ icon: Icon, color, title, desc, delay }: any) {
  ...
  return (
    <div ref={ref} style={{ padding: '28px 24px', background: 'rgba(22,34,53,0.6)', border: '1px solid rgba(30,58,95,0.5)', borderRadius: '18px', backdropFilter: 'blur(8px)', transition: `all .6s ${delay}ms cubic-bezier(.22,1,.36,1)`, opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(20px)', cursor: 'default' }}
      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = `${color}50`; (e.currentTarget as HTMLElement).style.transform = 'translateY(-3px)'; (e.currentTarget as HTMLElement).style.boxShadow = `0 12px 32px ${color}14`; }}
      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(30,58,95,0.5)'; (e.currentTarget as HTMLElement).style.transform = 'none'; (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}>
    ...
    <div style={{ background: 'rgba(10,18,30,0.9)', border: '1px solid rgba(30,58,95,0.6)', borderRadius: '20px', padding: '20px', backdropFilter: 'blur(20px)', boxShadow: '0 24px 60px rgba(0,0,0,.5),0 0 0 1px rgba(255,255,255,.03) inset', maxWidth: '310px', width: '100%' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
        {['Efectivo', 'SINPE', 'Tarjeta', 'Mixto'].map((m, i) => (
          <div key={m} style={{ padding: '8px', borderRadius: '9px', border: `1px solid ${i === 1 ? 'rgba(255,92,53,.4)' : 'rgba(30,58,95,0.5)'}`, background: i === 1 ? 'rgba(255,92,53,.08)' : 'transparent', textAlign: 'center' }}>
...
export default function LandingPage() {
  ...
  const featuresReveal = useReveal(), industriesReveal = useReveal(), stepsReveal = useReveal(), testimonialsReveal = useReveal(), ctaReveal = useReveal();
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060D16', fontFamily: 'Outfit, system-ui, sans-serif', overflowX: 'hidden' }}>
      ...
      {/* NAVBAR */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, height: '62px', borderBottom: '1px solid rgba(30,58,95,0.5)', background: 'rgba(6,13,22,0.88)', backdropFilter: 'blur(16px)', display: 'flex', alignItems: 'center', padding: '0 clamp(20px,5vw,80px)', justifyContent: 'space-between' }}>
        <Logo size="sm" />
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <a href="/login" style={{ padding: '7px 16px', borderRadius: '9px', fontSize: '13px', fontWeight: 600, color: 'rgba(176,208,240,.5)', textDecoration: 'none', transition: 'color .15s' }} onMouseEnter={e => (e.currentTarget.style.color = '#F0F4FF')} onMouseLeave={e => (e.currentTarget.style.color = 'rgba(176,208,240,.5)')}>Iniciar sesión</a>
          <a href="/register" style={{ padding: '8px 18px', borderRadius: '10px', fontSize: '13px', fontWeight: 700, color: '#fff', background: '#FF5C35', textDecoration: 'none', transition: 'all .18s', boxShadow: '0 2px 12px rgba(255,92,53,.35)' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#E8440E'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-1px)'; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = '#FF5C35'; (e.currentTarget as HTMLElement).style.transform = 'none'; }}>Empezar gratis</a>
        </div>
      </nav>
        ...
        <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '60px', flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 400px', opacity: mounted ? 1 : 0, transform: mounted ? 'none' : 'translateY(24px)', transition: 'all .9s cubic-bezier(.22,1,.36,1)' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '40px' }}>
              <a href="/register" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 28px', background: 'linear-gradient(135deg,#FF5C35,#FF3D1F)', color: '#fff', borderRadius: '13px', fontSize: '14px', fontWeight: 700, textDecoration: 'none', boxShadow: '0 4px 20px rgba(255,92,53,.4)', transition: 'all .2s' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 8px 28px rgba(255,92,53,.5)'; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'none'; (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 20px rgba(255,92,53,.4)'; }}>Empezar gratis <ArrowRight size={15} /></a>
              <a href="#precios" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 22px', background: 'transparent', color: '#4A7FAF', border: '1px solid rgba(30,58,95,.7)', borderRadius: '13px', fontSize: '14px', fontWeight: 600, textDecoration: 'none', transition: 'all .2s' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#2A5280'; (e.currentTarget as HTMLElement).style.background = 'rgba(30,58,95,.3)'; (e.currentTarget as HTMLElement).style.color = '#7BBEE8'; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(30,58,95,.7)'; (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = '#4A7FAF'; }}><Play size={13} /> Ver planes</a>
            </div>
            <div style={{ display: 'flex', gap: '28px', flexWrap: 'wrap' }}>
              {[{ value: '14 días', label: 'Prueba gratis' }, { value: 'Sin tarjeta', label: 'No requerida' }, { value: '8+', label: 'Industrias' }, { value: 'SINPE', label: 'Pago local' }].map(s => (
                <div key={s.label}><p style={{ fontSize: '17px', fontWeight: 800, color: '#fff', lineHeight: 1, marginBottom: '3px' }}>{s.value}</p><p style={{ fontSize: '10px', color: '#1E3A5F', letterSpacing: '.07em', textTransform: 'uppercase' }}>{s.label}</p></div>
              ))}
            </div>
          </div>
      ...
      <section style={{ padding: 'clamp(50px,6vh,80px) clamp(20px,5vw,80px)', borderTop: '1px solid rgba(30,58,95,.4)' }}>
        <div ref={industriesReveal.ref} style={{ maxWidth: '1100px', margin: '0 auto', opacity: industriesReveal.visible ? 1 : 0, transform: industriesReveal.visible ? 'none' : 'translateY(16px)', transition: 'all .7s cubic-bezier(.22,1,.36,1)' }}>
          <p style={{ textAlign: 'center', fontSize: '11px', fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: '#2A5280', marginBottom: '24px' }}>Una plataforma, todas las industrias</p>
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
            {INDUSTRIES.map(ind => {
              const Icon = ind.icon; return (
                <div key={ind.label} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '9px 18px', borderRadius: '12px', background: `${ind.color}0A`, border: `1px solid ${ind.color}20`, transition: 'all .18s', cursor: 'default' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = `${ind.color}18`; (e.currentTarget as HTMLElement).style.borderColor = `${ind.color}45`; (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)'; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = `${ind.color}0A`; (e.currentTarget as HTMLElement).style.borderColor = `${ind.color}20`; (e.currentTarget as HTMLElement).style.transform = 'none'; }}>
                  <Icon size={14} color={ind.color} strokeWidth={1.8} /><span style={{ fontSize: '13px', fontWeight: 600, color: ind.color }}>{ind.label}</span>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div ref={stepsReveal.ref} style={{ opacity: stepsReveal.visible ? 1 : 0, transform: stepsReveal.visible ? 'none' : 'translateY(16px)', transition: 'all .7s cubic-bezier(.22,1,.36,1)' }}>
            <div style={{ textAlign: 'center', marginBottom: '56px' }}>
              <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: '#FF5C35', marginBottom: '14px' }}>Cómo funciona</p>
              ...
              {STEPS.map((step, i) => {
                const Icon = step.icon; return (
                  <div key={i} style={{ padding: '28px 24px', background: 'rgba(22,34,53,0.5)', border: '1px solid rgba(30,58,95,.45)', borderRadius: '18px', backdropFilter: 'blur(8px)', position: 'relative', overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', top: '16px', right: '16px', fontSize: '48px', fontWeight: 900, color: `${step.color}06`, lineHeight: 1, userSelect: 'none' }}>{step.num}</div>
        ...
        <div style={{ maxWidth: '1060px', margin: '0 auto' }}>
          <div ref={testimonialsReveal.ref} style={{ opacity: testimonialsReveal.visible ? 1 : 0, transform: testimonialsReveal.visible ? 'none' : 'translateY(16px)', transition: 'all .7s cubic-bezier(.22,1,.36,1)' }}>
            <div style={{ textAlign: 'center', marginBottom: '48px' }}>
              <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: '#FF5C35', marginBottom: '14px' }}>Testimonios</p>
              ...
              {TESTIMONIALS.map((t, i) => (
                <div key={i} style={{ padding: '28px 24px', background: 'rgba(22,34,53,0.55)', border: '1px solid rgba(30,58,95,.45)', borderRadius: '18px', backdropFilter: 'blur(8px)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(255,92,53,.15)', border: '1px solid rgba(255,92,53,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><span style={{ fontSize: '13px', fontWeight: 700, color: '#FF5C35' }}>{t.name[0]}</span></div>
                    <div><p style={{ fontSize: '13px', fontWeight: 700, color: '#F0F4FF', lineHeight: 1, marginBottom: '2px' }}>{t.name}</p><p style={{ fontSize: '11px', color: '#2A5280' }}>{t.role} · {t.country}</p></div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ maxWidth: '1060px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: '16px' }}>
            {PLANS.map(plan => {
              ...
              return (
                <div key={plan.name} style={{ padding: '28px 24px', borderRadius: '22px', position: 'relative', background: plan.popular ? `linear-gradient(145deg,rgba(255,92,53,.07),rgba(10,18,30,.96))` : 'rgba(16,26,42,.85)', border: `1.5px solid ${plan.popular ? 'rgba(255,92,53,.35)' : 'rgba(30,58,95,.5)'}`, backdropFilter: 'blur(12px)', transition: 'all .2s' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLElement).style.boxShadow = `0 16px 40px ${plan.color}18`; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'none'; (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}>
                  {plan.popular && <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg,#FF5C35,#FF3D1F)', color: '#fff', fontSize: '10px', fontWeight: 800, padding: '4px 16px', borderRadius: '20px', whiteSpace: 'nowrap', letterSpacing: '.06em', textTransform: 'uppercase', boxShadow: '0 4px 12px rgba(255,92,53,.5)' }}>⚡ Más popular</div>}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px', marginTop: plan.popular ? '8px' : '0' }}>
                    ...
                    <div><p style={{ fontSize: '16px', fontWeight: 800, color: '#F0F4FF', lineHeight: 1, marginBottom: '2px' }}>{plan.label}</p><p style={{ fontSize: '11px', color: 'rgba(176,208,240,.4)', lineHeight: 1 }}>{plan.limit}</p></div>
                  </div>
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    {annual ? <p style={{ fontSize: '11px', color: '#3DBF7F', fontWeight: 600 }}>${plan.annualPrice}/año · ahorras ${saving}</p> : <p style={{ fontSize: '11px', color: 'rgba(176,208,240,.35)' }}>${plan.monthlyPrice * 12}/año facturado mensual</p>}
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <a href="/register" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '13px', background: plan.popular ? `linear-gradient(135deg,${plan.color},#FF3D1F)` : `${plan.color}18`, border: `1.5px solid ${plan.color}`, borderRadius: '13px', color: plan.popular ? '#fff' : plan.color, fontSize: '13px', fontWeight: 700, textDecoration: 'none', transition: 'all .15s' }}>Empezar gratis <ArrowRight size={13} /></a>
                    <button onClick={() => setSinpePlan(plan as any)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '10px', background: 'transparent', border: '1px solid rgba(30,58,95,.5)', borderRadius: '13px', color: 'rgba(176,208,240,.45)', fontSize: '12px', fontWeight: 600, cursor: 'pointer', transition: 'all .15s', fontFamily: 'Outfit, sans-serif' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = plan.color; (e.currentTarget as HTMLElement).style.color = plan.color; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(30,58,95,.5)'; (e.currentTarget as HTMLElement).style.color = 'rgba(176,208,240,.45)'; }}>
      ...
      <section style={{ padding: 'clamp(80px,10vh,120px) clamp(20px,5vw,80px)', borderTop: '1px solid rgba(30,58,95,.4)' }}>
        <div ref={ctaReveal.ref} style={{ maxWidth: '640px', margin: '0 auto', textAlign: 'center', opacity: ctaReveal.visible ? 1 : 0, transform: ctaReveal.visible ? 'none' : 'translateY(20px)', transition: 'all .7s cubic-bezier(.22,1,.36,1)' }}>
          ...
          <a href="/register" style={{ display: 'inline-flex', alignItems: 'center', gap: '9px', padding: '16px 36px', background: 'linear-gradient(135deg,#FF5C35,#FF3D1F)', color: '#fff', borderRadius: '14px', fontSize: '15px', fontWeight: 700, textDecoration: 'none', boxShadow: '0 4px 24px rgba(255,92,53,.45)', transition: 'all .2s' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 8px 32px rgba(255,92,53,.55)'; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'none'; (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 24px rgba(255,92,53,.45)'; }}>
      ...
      <footer style={{ borderTop: '1px solid rgba(30,58,95,.4)', padding: '28px clamp(20px,5vw,80px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          {[{ label: 'Términos', href: '/terms' }, { label: 'Privacidad', href: '/privacy' }, { label: 'Login', href: '/login' }, { label: 'Registro', href: '/register' }].map(l => (
            <a key={l.label} href={l.href} style={{ fontSize: '12px', color: '#2A5280', textDecoration: 'none', transition: 'color .15s' }} onMouseEnter={e => (e.currentTarget.style.color = '#4A7FAF')} onMouseLeave={e => (e.currentTarget.style.color = '#2A5280')}>{l.label}</a>
          ))}
        </div>
