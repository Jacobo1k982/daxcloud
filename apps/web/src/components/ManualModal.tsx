...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    chapters: [
      {
        id: 'pedidos-intro',
        title: '¿Qué es el catálogo online?',
        content: (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '16px' }}>
            <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.6)', lineHeight: 1.85 }}>
              El catálogo online es una página pública donde tus clientes pueden ver tus productos y hacer pedidos directamente desde su celular, sin necesidad de descargar ninguna app.
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '10px' }}>
              {[
                { icon: Globe, title: 'Link público', desc: 'Cada negocio tiene su propia URL para compartir' },
                { icon: Smartphone, title: '100% móvil', desc: 'Optimizado para celular, tablet y desktop' },
                { icon: Bell, title: 'Tiempo real', desc: 'Recibes notificación instantánea al llegar un pedido' },
                { icon: CreditCard, title: 'Contra entrega', desc: 'El cliente paga al recibir o recoger su pedido' },
              ].map(({ icon: Icon, title, desc }) => (
                <div key={title} style={{ padding: '14px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                    <Icon size={14} color="#FF5C35" />
            ...
            <Tip>El link de tu catálogo tiene el formato: <strong style={{ color: '#FF5C35' }}>daxcloud.shop/order/tu-negocio</strong> — puedes compartirlo por WhatsApp, Instagram o imprimirlo en un QR.</Tip>
          </div>
        ),
      },
      {
        id: 'pedidos-link',
        title: 'Compartir tu link de pedidos',
        content: (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7 }}>Encuentra y comparte tu link desde el dashboard:</p>
            <Step n={1} title="Ve a Ventas en el sidebar" desc="En el menú lateral selecciona el módulo de Ventas." />
            <Step n={2} title="Selecciona el tab Pedidos online" desc="Verás el tab 'Pedidos online' junto a los demás filtros de ventas." />
            <Step n={3} title="Copia tu link" desc="En la sección superior aparece tu link público. Toca 'Copiar' para copiarlo al portapapeles." />
            <Step n={4} title="Comparte con tus clientes" desc="Pégalo en WhatsApp, bio de Instagram, Facebook o imprímelo como código QR para tu local." />
            <Tip>También puedes tocar 'Ver página' para previsualizar cómo ve el catálogo tu cliente antes de compartirlo.</Tip>
          </div>
        ),
      },
      {
        id: 'pedidos-flujo-cliente',
        title: 'Flujo del cliente',
        content: (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7 }}>Así vive el proceso tu cliente:</p>
            <Step n={1} title="Abre el link" desc="El cliente abre el link desde su celular — no necesita registrarse ni descargar nada." />
            <Step n={2} title="Explora el catálogo" desc="Ve todos los productos con foto, nombre y precio. Puede buscar y filtrar por categoría." />
            <Step n={3} title="Agrega al carrito" desc="Toca el botón + para agregar productos. Puede ajustar cantidades o eliminar ítems." />
            ...
            <Note>El pago es siempre contra entrega — el cliente paga cuando recoge o recibe su pedido. No se procesa ningún pago online en este flujo.</Note>
          </div>
        ),
      },
      {
        ...
        content: (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '14px' }}>
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7 }}>Cuando llega un pedido nuevo recibes una notificación y aparece en tu dashboard:</p>
            <Table
              headers={['Estado', 'Color', 'Acción siguiente']}
              rows={[
                ['Nuevo', '🟠 Coral', 'Confirmar y empezar a preparar'],
                ['Preparando', '🟡 Ámbar', 'Avisar al cliente que está listo'],
                ['Listo', '🔵 Azul', 'Marcar como entregado'],
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '8px', marginTop: '4px' }}>
              {[
                { badge: 'Ver detalle', color: 'rgba(255,255,255,0.4)', desc: 'Muestra los productos, datos del cliente y notas del pedido.' },
                { badge: 'Llamar', color: '#5AAAF0', desc: 'Abre el marcador con el teléfono del cliente para contactarlo.' },
                { badge: 'Avanzar', color: '#FF5C35', desc: 'Mueve el pedido al siguiente estado del flujo.' },
                { badge: 'Cancelar', color: '#E07070', desc: 'Cancela el pedido si no se puede procesar.' },
              ].map(({ badge, color, desc }) => (
                <div key={badge} style={{ display: 'flex', gap: '12px', alignItems: 'center', padding: '10px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '10px' }}>
                  <Badge label={badge} color={color} />
                  <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.45)' }}>{desc}</p>
                </div>
              ))}
            </div>
            <Tip>Los pedidos con estado Nuevo parpadean en coral para que no los pierdas. El dashboard se actualiza automáticamente cada 15 segundos.</Tip>
          </div>
        ),
      },
      {
        id: 'pedidos-productos',
        title: 'Qué productos aparecen en el catálogo',
        content: (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '14px' }}>
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7 }}>El catálogo público muestra automáticamente todos los productos que tengas configurados en tu negocio con estas condiciones:</p>
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '4px' }}>
              <KeyVal k="Estado" v="Solo productos activos" />
              <KeyVal k="Imagen" v="Si tiene foto se muestra, si no un ícono de placeholder" />
              <KeyVal k="Categoría" v="Se usan como filtros en el catálogo" />
              <KeyVal k="Precio" v="El precio de venta configurado en el producto" />
              <KeyVal k="Descripción" v="Si tiene descripción se muestra debajo del nombre" />
            </div>
            <Note>Para que un producto aparezca en el catálogo debe estar activo. Ve a Productos → edita el producto → verifica que esté marcado como Activo.</Note>
            <Tip>Agrega fotos y descripciones a tus productos para mejorar la experiencia del cliente en el catálogo online.</Tip>
          </div>
        ),
      },
    ],
