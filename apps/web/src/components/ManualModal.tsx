...
function Step({ n, title, desc }: { n:number; title:string; desc:string }) {
  return (
    <div style={{ display:'flex', gap:'14px', alignItems:'flex-start', padding:'13px 14px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'11px' }}>
      <div style={{ width:'28px', height:'28px', borderRadius:'8px', background:'linear-gradient(135deg,#FF5C35,#FF3D1F)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'12px', fontWeight:800, color: 'var(--dax-text-primary)' }}>{n}</div>
      <div>
        <p style={{ fontSize:'13px', fontWeight:700, color:'var(--dax-text-primary)', marginBottom:'3px' }}>{title}</p>
        <p style={{ fontSize:'12px', color:'var(--dax-white-35)', lineHeight:1.6 }}>{desc}</p>
      </div>
    </div>
  );
}
...
function Tip({ children }: { children:React.ReactNode }) {
  return (
    <div style={{ display:'flex', gap:'10px', padding:'11px 13px', background:'var(--dax-coral-soft)', border:'1px solid rgba(255,92,53,0.18)', borderRadius:'9px', marginTop:'10px' }}>
      <Star size={13} color="#FF5C35" style={{ flexShrink:0, marginTop:'1px' }}/>
      <p style={{ fontSize:'12px', color:'var(--dax-white-60)', lineHeight:1.65 }}>{children}</p>
    </div>
  );
}
...
function Note({ children, color='#5AAAF0' }: { children:React.ReactNode; color?:string }) {
  return (
    <div style={{ display:'flex', gap:'10px', padding:'11px 13px', background:`rgba(90,170,240,0.06)`, border:`1px solid rgba(90,170,240,0.15)`, borderRadius:'9px', marginTop:'10px' }}>
      ...
      <p style={{ fontSize:'12px', color:'var(--dax-white-35)', lineHeight:1.65 }}>{children}</p>
    </div>
  );
}
...
function Warning({ children }: { children:React.ReactNode }) {
  return (
    <div style={{ display:'flex', gap:'10px', padding:'11px 13px', background:'var(--dax-warning-bg)', border:'1px solid rgba(240,160,48,0.2)', borderRadius:'9px', marginTop:'10px' }}>
...
function KeyVal({ k, v }: { k:string; v:string }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'16px', padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
      <span style={{ fontSize:'12px', color:'var(--dax-white-35)', flexShrink:0 }}>{k}</span>
      <span style={{ fontSize:'12px', fontWeight:600, color:'var(--dax-white-60)', textAlign:'right' as const }}>{v}</span>
    </div>
  );
}
...
function Table({ headers, rows }: { headers:string[]; rows:string[][] }) {
  return (
    <div style={{ borderRadius:'11px', overflow:'hidden', border:'1px solid rgba(255,255,255,0.07)', marginTop:'10px' }}>
      <div style={{ display:'grid', gridTemplateColumns:`repeat(${headers.length},1fr)`, background:'var(--dax-surface-2)', padding:'9px 13px' }}>
      ...
      {rows.map((row,i) => (
        <div key={i} style={{ display:'grid', gridTemplateColumns:`repeat(${headers.length},1fr)`, padding:'9px 13px', background: i%2===0?'transparent':'rgba(255,255,255,0.015)', borderTop:'1px solid rgba(255,255,255,0.04)' }}>
          {row.map((cell,j) => <span key={j} style={{ fontSize:'12px', color: j===0?'var(--dax-white-60)':'var(--dax-white-35)', fontWeight: j===0?600:400 }}>{cell}</span>)}
        </div>
      ))}
    </div>
  );
...
function MiniCard({ icon: Icon, title, desc, color='#FF5C35' }: { icon:any; title:string; desc:string; color?:string }) {
  return (
    <div style={{ padding:'13px', background:'var(--dax-surface)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'11px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'5px' }}>
        <Icon size={13} color={color}/>
        <span style={{ fontSize:'12px', fontWeight:700, color:'var(--dax-text-primary)' }}>{title}</span>
      </div>
      <p style={{ fontSize:'11px', color: 'var(--dax-white-35)', lineHeight:1.5 }}>{desc}</p>
    </div>
  );
}
...
const SECTIONS: Section[] = [
  ...
  // ── 1. INTRODUCCIÓN ────────────────────────────────────────────────────────
  {
    id:'inicio', icon:BookOpen, label:'Introducción', color: 'var(--dax-coral)',
    chapters:[
      {
        id:'que-es', title:'¿Qué es DaxCloud?',
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
            <p style={{ fontSize:'14px', color:'var(--dax-white-60)', lineHeight:1.85 }}>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7, marginBottom:'4px' }}>Configura tu negocio en menos de 5 minutos:</p>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>DaxCloud se puede instalar como aplicación nativa en cualquier dispositivo sin pasar por el App Store ni Google Play.</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
              {[
                ...
                <div key={plat} style={{ padding:'11px 13px', background:'var(--dax-surface)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'10px' }}>
                  <p style={{ fontSize:'12px', fontWeight:700, color:'var(--dax-text-primary)', marginBottom:'4px' }}>{plat}</p>
                  <p style={{ fontSize:'11px', color:'var(--dax-white-35)', lineHeight:1.5 }}>{steps}</p>
                </div>
              ))}
            </div>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
            ...
            />
            <Grid2>
              <div style={{ padding:'13px', background:'var(--dax-success-bg)', border:'1px solid rgba(61,191,127,0.15)', borderRadius:'11px' }}>
                <p style={{ fontSize:'11px', color:'rgba(61,191,127,0.7)', fontWeight:600, marginBottom:'5px' }}>Facturación anual</p>
                <p style={{ fontSize:'18px', fontWeight:800, color: 'var(--dax-success)' }}>2 meses gratis</p>
                <p style={{ fontSize:'11px', color: 'var(--dax-white-35)', marginTop:'3px' }}>Ahorra hasta $120/año</p>
              </div>
              <div style={{ padding:'13px', background:'var(--dax-coral-soft)', border:'1px solid rgba(255,92,53,0.15)', borderRadius:'11px' }}>
                <p style={{ fontSize:'11px', color:'rgba(255,92,53,0.7)', fontWeight:600, marginBottom:'5px' }}>Módulos especiales</p>
                <p style={{ fontSize:'18px', fontWeight:800, color: 'var(--dax-coral)' }}>+$22/mes</p>
                <p style={{ fontSize:'11px', color: 'var(--dax-white-35)', marginTop:'3px' }}>Por industria especializada</p>
              </div>
            </Grid2>
  ...
  // ── 2. POS ─────────────────────────────────────────────────────────────────
  {
    id:'pos', icon:ShoppingCart, label:'Punto de Venta', color: 'var(--dax-coral)',
    chapters:[
      {
        id:'pos-apertura', title:'Apertura de caja',
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>Antes de vender debes abrir la caja con el monto inicial de efectivo disponible.</p>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>El POS tiene 3 áreas principales:</p>
            {[
              { area:'Panel de productos', desc:'Búsqueda por nombre o código de barras, filtros por categoría, grid de productos con imágenes y precios', color: 'var(--dax-coral)' },
              { area:'Carrito / Orden', desc:'Productos agregados, cantidades, descuentos por ítem, descuento global y total', color: 'var(--dax-blue)' },
              { area:'Barra de caja', desc:'Estadísticas del turno, botones de apertura/cierre de caja, refrescar inventario', color: 'var(--dax-success)' },
            ].map(({ area, desc, color }) => (
              <div key={area} style={{ display:'flex', gap:'12px', padding:'11px 13px', background:'rgba(255,255,255,0.02)', border:`1px solid ${color}20`, borderRadius:'10px' }}>
                ...
                <div><p style={{ fontSize:'12px', fontWeight:700, color:'var(--dax-text-primary)', marginBottom:'3px' }}>{area}</p><p style={{ fontSize:'11px', color: 'var(--dax-white-35)', lineHeight:1.5 }}>{desc}</p></div>
              </div>
            ))}
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>Después de cada venta puedes generar el comprobante:</p>
            {[
              ...
              <div key={title} style={{ display:'flex', gap:'12px', padding:'11px 13px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'10px' }}>
                ...
                <div><p style={{ fontSize:'12px', fontWeight:700, color:'var(--dax-text-primary)', marginBottom:'3px' }}>{title}</p><p style={{ fontSize:'11px', color: 'var(--dax-white-35)', lineHeight:1.5 }}>{desc}</p></div>
              </div>
            ))}
  ...
  // ── 3. PRODUCTOS ───────────────────────────────────────────────────────────
  {
    id:'productos', icon:Package, label:'Productos', color: 'var(--dax-blue)',
    chapters:[
      {
        id:'prod-crear', title:'Crear un producto',
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>El formulario de productos tiene 4 pestañas:</p>
            {[
              ...
              <div key={tab} style={{ display:'flex', gap:'12px', padding:'10px 13px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,92,53,0.15)', borderRadius:'10px' }}>
                ...
                <p style={{ fontSize:'12px', color:'var(--dax-white-35)', lineHeight:1.5, flex:1 }}>{desc}</p>
              </div>
            ))}
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>DaxCloud soporta múltiples tasas de IVA por producto:</p>
            <Table
            ...
            />
            <div style={{ padding:'12px 14px', background:'var(--dax-info-bg)', border:'1px solid rgba(90,170,240,0.15)', borderRadius:'10px' }}>
              <p style={{ fontSize:'12px', fontWeight:700, color:'rgba(90,170,240,0.8)', marginBottom:'6px' }}>Toggle "Precio incluye IVA"</p>
              <p style={{ fontSize:'11px', color:'var(--dax-white-35)', lineHeight:1.6 }}><strong style={{ color: 'var(--dax-white-60)' }}>Activado:</strong> El precio ingresado ya tiene IVA incluido. Se desglosará automáticamente para reportes.<br/><strong style={{ color: 'var(--dax-white-60)' }}>Desactivado:</strong> El IVA se suma al precio de venta al momento del cobro.</p>
            </div>
  ...
  // ── 4. INVENTARIO ──────────────────────────────────────────────────────────
  {
    id:'inventario', icon:Package, label:'Inventario', color: 'var(--dax-blue)',
    chapters:[
      {
        id:'inv-productos', title:'Gestión de stock',
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>El inventario se actualiza automáticamente con cada venta. Para ajustes manuales:</p>
            {[
              { badge:'Entrada',  color: 'var(--dax-success)', desc:'Registra compras a proveedores o reposición de stock.' },
              { badge:'Salida',   color: 'var(--dax-danger)', desc:'Ajuste por pérdida, merma o daño de productos.' },
              { badge:'Ajuste',   color: 'var(--dax-amber)', desc:'Corrección después de conteo físico.' },
              { badge:'Traslado', color: 'var(--dax-blue)', desc:'Mover stock entre sucursales (plan Growth+).' },
              ...
              <div key={badge} style={{ display:'flex', gap:'12px', alignItems:'center', padding:'9px 13px', background:'rgba(255,255,255,0.02)', border:`1px solid ${color}20`, borderRadius:'9px' }}>
                ...
                <p style={{ fontSize:'12px', color:'var(--dax-white-35)' }}>{desc}</p>
              </div>
            ))}
          </div>
        ),
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {[
              { icon:Bell, title:'Stock bajo', color: 'var(--dax-amber)', desc:'Cuando un producto llega a su mínimo configurado. Aparece en el POS y en notificaciones.' },
              { icon:Bell, title:'Producto agotado', color: 'var(--dax-danger)', desc:'Notificación inmediata cuando el stock llega a cero. El producto se marca como AGOTADO en el POS.' },
              { icon:Bell, title:'Lote por vencer', color: 'var(--dax-purple)', desc:'Para farmacia y verdulería, alerta de productos próximos a vencer (configurable por días).' },
  ...
  // ── 5. ANALYTICS ───────────────────────────────────────────────────────────
  {
    id:'analytics', icon:BarChart2, label:'Analytics', color: 'var(--dax-success)',
    chapters:[
      {
            ...
            <Grid2>
              {[
                ...
                <div key={label} style={{ padding:'11px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'10px' }}>
                  <p style={{ fontSize:'11px', fontWeight:700, color: 'var(--dax-success)', marginBottom:'3px' }}>{label}</p>
                  <p style={{ fontSize:'11px', color: 'var(--dax-white-35)' }}>{value}</p>
                </div>
              ))}
            </Grid2>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {[
              ...
              <div key={title} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'10px 13px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'10px' }}>
                ...
                <div><p style={{ fontSize:'12px', fontWeight:700, color:'var(--dax-white-60)', marginBottom:'2px' }}>{title}</p><p style={{ fontSize:'11px', color: 'var(--dax-white-35)' }}>{desc}</p></div>
              </div>
            ))}
          </div>
        ),
  ...
  // ── 6. CONTABILIDAD ────────────────────────────────────────────────────────
  {
    id:'contabilidad', icon:Scale, label:'Contabilidad PRO', color: 'var(--dax-purple)',
    chapters:[
      {
        id:'cont-overview', title:'¿Qué incluye el módulo contable?',
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>El módulo contable de DaxCloud está diseñado para contadores profesionales. Incluye 6 vistas especializadas:</p>
            {[
              { icon:LayoutDashboard, label:'Dashboard', color: 'var(--dax-coral)', desc:'KPIs financieros, métodos de pago con barras, P&L rápido y gráfico de ingresos por día.' },
              { icon:BookOpen,        label:'Libro de ventas', color: 'var(--dax-blue)', desc:'Tabla completa de transacciones con búsqueda, exportar a Excel y PDF profesional para Hacienda.' },
              { icon:TrendingUp,      label:'Estado de resultados', color: 'var(--dax-success)', desc:'Desglose contable: ingresos, costos, utilidad, IVA e indicadores financieros.' },
              { icon:Receipt,        label:'Declaración IVA', color: 'var(--dax-purple)', desc:'Base imponible, IVA cobrado y tasa efectiva por día. Listo para declarar ante Hacienda.' },
              { icon:Activity,       label:'Flujo de caja', color: 'var(--dax-amber)', desc:'Ingresos, costos, utilidad neta y saldo acumulado día a día.' },
              { icon:PieChart,       label:'Análisis de productos', color: 'var(--dax-danger)', desc:'Ranking de productos por ingresos, margen, unidades vendidas y % del total.' },
              ...
              <div key={label} style={{ display:'flex', gap:'12px', padding:'11px 13px', background:'rgba(255,255,255,0.02)', border:`1px solid ${color}20`, borderRadius:'10px' }}>
                ...
                <div><p style={{ fontSize:'12px', fontWeight:700, color:'var(--dax-text-primary)', marginBottom:'3px' }}>{label}</p><p style={{ fontSize:'11px', color: 'var(--dax-white-35)', lineHeight:1.5 }}>{desc}</p></div>
              </div>
            ))}
          </div>
        ),
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>Cada reporte contable se puede exportar en dos formatos:</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
              <div style={{ padding:'13px', background:'var(--dax-success-bg)', border:'1px solid rgba(61,191,127,0.15)', borderRadius:'11px' }}>
                <p style={{ fontSize:'12px', fontWeight:700, color: 'var(--dax-success)', marginBottom:'5px' }}>Excel / CSV</p>
                <p style={{ fontSize:'11px', color:'var(--dax-white-35)', lineHeight:1.6 }}>Archivo con todos los datos numéricos para importar en Excel, Google Sheets o el software contable del contador. Incluye encabezados en español y formato de moneda costarricense.</p>
              </div>
              <div style={{ padding:'13px', background:'var(--dax-coral-soft)', border:'1px solid rgba(255,92,53,0.15)', borderRadius:'11px' }}>
                <p style={{ fontSize:'12px', fontWeight:700, color: 'var(--dax-coral)', marginBottom:'5px' }}>PDF imprimible</p>
                <p style={{ fontSize:'11px', color:'var(--dax-white-35)', lineHeight:1.6 }}>Reporte con encabezado profesional, nombre del negocio, período y fecha de generación. Listo para imprimir o enviar al contador por correo.</p>
              </div>
            </div>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>Pasos para preparar la declaración mensual de IVA:</p>
  ...
  // ── 7. CLIENTES ────────────────────────────────────────────────────────────
  {
    id:'clientes', icon:Users, label:'Clientes', color: 'var(--dax-purple)',
    chapters:[
      {
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {[
              { icon:Star, title:'Puntos de fidelización', color: 'var(--dax-amber)', desc:'Configura cuántos puntos se otorgan por cada ₡1,000 vendidos. Los puntos se canjean en futuras compras como descuento.' },
              { icon:CreditCard, title:'Crédito interno', color: 'var(--dax-blue)', desc:'Asigna un límite de crédito al cliente. Puede comprar a cuenta y pagar después desde Clientes → Pagar crédito.' },
              { icon:TrendingUp, title:'Historial y análisis', color: 'var(--dax-success)', desc:'Ve el valor total comprado, frecuencia de visitas, productos favoritos y comportamiento de compra.' },
            ].map(({ icon:Icon, title, color, desc }) => (
              <div key={title} style={{ display:'flex', gap:'12px', padding:'12px 13px', background:'rgba(255,255,255,0.02)', border:`1px solid ${color}20`, borderRadius:'10px' }}>
                ...
                <div><p style={{ fontSize:'12px', fontWeight:700, color:'var(--dax-text-primary)', marginBottom:'4px' }}>{title}</p><p style={{ fontSize:'11px', color: 'var(--dax-white-35)', lineHeight:1.55 }}>{desc}</p></div>
              </div>
            ))}
          </div>
        ),
  ...
  // ── 8. INDUSTRIAS ──────────────────────────────────────────────────────────
  {
    id:'industrias', icon:Building2, label:'Módulos de industria', color: 'var(--dax-amber)',
    chapters:[
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'4px' }}>
              <div style={{ width:'34px', height:'34px', borderRadius:'9px', background:'var(--dax-warning-bg)', display:'flex', alignItems:'center', justifyContent:'center' }}><ChefHat size={17} color="#F0A030"/></div>
              <div><p style={{ fontSize:'14px', fontWeight:700, color: 'var(--dax-text-primary)' }}>Módulo Restaurante</p><p style={{ fontSize:'11px', color:'rgba(240,160,48,0.7)' }}>Comedor completo con comandas</p></div>
            </div>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'4px' }}>
              <div style={{ width:'34px', height:'34px', borderRadius:'9px', background: 'var(--dax-coral-soft)', display:'flex', alignItems:'center', justifyContent:'center' }}><ChefHat size={17} color="#FF5C35"/></div>
              <div><p style={{ fontSize:'14px', fontWeight:700, color: 'var(--dax-text-primary)' }}>Módulo Panadería</p><p style={{ fontSize:'11px', color:'rgba(255,92,53,0.7)' }}>Producción y encargos</p></div>
            </div>
            <Table
              headers={['Tab','Función']}
              rows={[
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'4px' }}>
              <div style={{ width:'34px', height:'34px', borderRadius:'9px', background:'var(--dax-success-bg)', display:'flex', alignItems:'center', justifyContent:'center' }}><Pill size={17} color="#3DBF7F"/></div>
              <div><p style={{ fontSize:'14px', fontWeight:700, color: 'var(--dax-text-primary)' }}>Módulo Farmacia</p><p style={{ fontSize:'11px', color:'rgba(61,191,127,0.7)' }}>Medicamentos y control de lotes</p></div>
            </div>
            <div style={{ display:'flex', flexWrap:'wrap' as const }}>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {[
              { icon:Scissors, title:'Peluquería / Salón', color: 'var(--dax-purple)', features:['Agenda de citas','Servicios por empleado','Comisiones','Historial del cliente'] },
              { icon:Shirt,    title:'Ropa / Moda',        color: 'var(--dax-blue)', features:['Tallas y colores','Colecciones','Temporadas','Devoluciones'] },
              { icon:Leaf,     title:'Verdulería',         color: 'var(--dax-success)', features:['Venta por peso','Control de frescura','Lotes','Precios de temporada'] },
              { icon:ShoppingBag, title:'Supermercado',   color: 'var(--dax-amber)', features:['Código de barras','Precios por volumen','Cajas múltiples','Proveedores'] },
              ...
              <div key={title} style={{ padding:'13px', background:'rgba(255,255,255,0.02)', border:`1px solid ${color}18`, borderRadius:'11px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'10px' }}>
                  ...
                  <p style={{ fontSize:'13px', fontWeight:700, color:'var(--dax-text-primary)' }}>{title}</p>
                </div>
  ...
  // ── 9. PEDIDOS ONLINE ──────────────────────────────────────────────────────
  {
    id:'pedidos', icon:Globe, label:'Pedidos online', color: 'var(--dax-coral)',
    chapters:[
      {
        id:'pedidos-intro', title:'¿Qué es el catálogo online?',
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
            <p style={{ fontSize:'14px', color:'var(--dax-white-60)', lineHeight:1.85 }}>El catálogo online es una página pública donde tus clientes ven tus productos y hacen pedidos directamente desde su celular, sin descargar ninguna app.</p>
            <Grid2>
            ...
            <Tip>El link de tu catálogo es: <strong style={{ color: 'var(--dax-coral)' }}>daxcloud.shop/order/tu-slug</strong>. Puedes compartirlo por WhatsApp, Instagram o como código QR.</Tip>
          </div>
        ),
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>El catálogo usa la información de tu perfil público:</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            ...
            />
            <p style={{ fontSize:'12px', color:'var(--dax-white-35)', lineHeight:1.7 }}>Los pedidos nuevos pulsan en coral para que no los pierdas. El dashboard se actualiza cada 15 segundos automáticamente.</p>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>El catálogo muestra automáticamente todos los productos activos con:</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
  ...
  // ── 10. PERFIL PÚBLICO ─────────────────────────────────────────────────────
  {
    id:'perfil', icon:Palette, label:'Perfil del negocio', color: 'var(--dax-amber)',
    chapters:[
      {
        id:'perfil-config', title:'Configurar el perfil público',
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>Ve a <strong style={{ color: 'var(--dax-text-primary)' }}>Configuración → Perfil público</strong> para editar toda la información visible para tus clientes.</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
  ...
  // ── 11. CONFIGURACIÓN ──────────────────────────────────────────────────────
  {
    id:'configuracion', icon:Settings, label:'Configuración', color: 'var(--dax-blue)',
    chapters:[
      {
        id:'config-empresa', title:'Datos de empresa',
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>En Configuración encuentras todas las secciones del sistema:</p>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>Cada sucursal tiene su propio:</p>
            <div style={{ display:'flex', flexWrap:'wrap' as const, gap:'6px' }}>
      ...
      },
      {
        ...
        content:(
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <p style={{ fontSize:'13px', color:'var(--dax-white-60)', lineHeight:1.7 }}>DaxCloud implementa múltiples capas de seguridad:</p>
            {[
  ...
  {
    id:'pagos', icon:CreditCard, label:'Suscripción y pagos', color: 'var(--dax-success)',
    chapters:[
      {
...
export function ManualModal({ onClose }: { onClose:() => void }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', padding:'16px', fontFamily:"'Inter',system-ui,sans-serif" }}>
      <div onClick={onClose} style={{ position:'absolute', inset:0, background: 'var(--dax-overlay)', backdropFilter:'blur(8px)' }}/>

      <div style={{ position:'relative', width:'100%', maxWidth:'1120px', height:'min(92vh,780px)', background: 'var(--dax-bg)', border:'1px solid rgba(255,92,53,0.18)', borderRadius:'20px', display:'flex', flexDirection:'column', overflow:'hidden', boxShadow:'0 32px 100px rgba(0,0,0,0.8)', animation:'manualOpen .35s cubic-bezier(.22,1,.36,1)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'14px', padding:'14px 22px', borderBottom:'1px solid rgba(255,255,255,0.06)', flexShrink:0, background:'var(--dax-surface)', backdropFilter:'blur(20px)', position:'relative', zIndex:2 }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px', flex:1 }}>
            ...
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:'7px' }}>
                <h2 style={{ fontSize:'15px', fontWeight:800, color: 'var(--dax-text-primary)', letterSpacing:'-.02em', margin:0 }}>Manual de DaxCloud</h2>
                <span style={{ fontSize:'9px', fontWeight:700, padding:'2px 7px', borderRadius:'20px', background: 'var(--dax-coral-soft)', border:'1px solid rgba(255,92,53,0.2)', color: 'var(--dax-coral)' }}>v3.0</span>
              </div>
              <p style={{ fontSize:'10px', color:'var(--dax-text-muted)', margin:0 }}>{SECTIONS.length} secciones · {totalChapters} capítulos</p>
            </div>
          </div>
          ...
          <div style={{ position:'relative', flex:1, maxWidth:'280px' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar en el manual..."
              style={{ width:'100%', padding:'8px 13px 8px 32px', background:'var(--dax-surface-2)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'9px', color: 'var(--dax-text-primary)', fontSize:'12px', fontFamily:'inherit', outline:'none', boxSizing:'border-box' as const, transition:'all .2s' }}
            ...
            {search.length > 2 && (
              <div style={{ position:'absolute', top:'calc(100% + 5px)', left:0, right:0, background:'rgba(8,14,26,0.99)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'11px', overflow:'hidden', zIndex:100, boxShadow:'0 16px 40px rgba(0,0,0,0.6)', maxHeight:'240px', overflowY:'auto' }}>
                {searchResults.length === 0
                  ? <p style={{ padding:'13px 15px', fontSize:'12px', color:'var(--dax-text-muted)' }}>Sin resultados</p>
                  : searchResults.map(({ section, chapter }) => (
                    <div key={chapter.id} onClick={() => selectChapter(section.id, chapter.id)}
                      ...
                      <div>
                        <p style={{ fontSize:'12px', fontWeight:600, color:'var(--dax-text-primary)', marginBottom:'1px' }}>{chapter.title}</p>
                        <p style={{ fontSize:'10px', color:'var(--dax-text-muted)' }}>{section.label}</p>
                      </div>
                    </div>
                  ))
                }
          ...
          <button onClick={() => setMobileNav(p => !p)} className="manual-toggle" style={{ display:'none', alignItems:'center', gap:'5px', padding:'7px 11px', background:'var(--dax-coral-soft)', border:'1px solid rgba(255,92,53,0.2)', borderRadius:'8px', color: 'var(--dax-coral)', fontSize:'11px', fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
            <Layers size={12}/> Secciones
          </button>

          <button onClick={onClose} style={{ width:'30px', height:'30px', borderRadius:'8px', background:'var(--dax-surface-2)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'all .15s' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background='var(--dax-coral-soft)'; }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background='var(--dax-surface-2)'; }}>
        ...
        <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
              ...
              return (
                <div key={section.id} style={{ marginBottom:'3px' }}>
                  <div onClick={() => { setActiveSection(section.id); setActiveChapter(section.chapters[0].id); setMobileNav(false); }}
                    ...
                    onMouseEnter={e => { if(!isActive) (e.currentTarget as HTMLElement).style.background='var(--dax-surface)'; }}
                    onMouseLeave={e => { if(!isActive) (e.currentTarget as HTMLElement).style.background='transparent'; }}>
                    <Icon size={13} color={isActive?section.color:'var(--dax-text-muted)'}/>
                    <span style={{ fontSize:'11px', fontWeight: isActive?700:500, color: isActive?section.color:'var(--dax-white-35)', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' as const }}>{section.label}</span>
                    ...
                    <div style={{ paddingLeft:'12px', display:'flex', flexDirection:'column', gap:'1px' }}>
                      {section.chapters.map(chapter => (
                        <div key={chapter.id} onClick={() => { setActiveChapter(chapter.id); setMobileNav(false); }}
                          style={{ display:'flex', alignItems:'center', gap:'7px', padding:'6px 9px', borderRadius:'7px', cursor:'pointer', background: activeChapter===chapter.id?'var(--dax-surface-2)':'transparent', transition:'background .12s' }}
                          onMouseEnter={e => { if(activeChapter!==chapter.id) (e.currentTarget as HTMLElement).style.background='var(--dax-surface)'; }}
                          onMouseLeave={e => { if(activeChapter!==chapter.id) (e.currentTarget as HTMLElement).style.background='transparent'; }}>
                          <div style={{ width:'4px', height:'4px', borderRadius:'50%', background: activeChapter===chapter.id?section.color:'var(--dax-text-muted)', flexShrink:0, transition:'background .15s' }}/>
                          <span style={{ fontSize:'11px', color: activeChapter===chapter.id?'#fff':'var(--dax-white-35)', fontWeight: activeChapter===chapter.id?600:400, lineHeight:1.4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' as const }}>{chapter.title}</span>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <div ref={contentRef} style={{ flex:1, overflowY:'auto', padding:'24px 28px' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'5px', marginBottom:'18px' }}>
              <span style={{ fontSize:'11px', color: 'var(--dax-white-25)' }}>{currentSection.label}</span>
              <ChevronRight size={9} color="rgba(255,255,255,0.2)"/>
              <span style={{ fontSize:'11px', color:'var(--dax-white-60)', fontWeight:600 }}>{currentChapter?.title}</span>
              <span style={{ marginLeft:'auto', fontSize:'10px', color:'var(--dax-text-muted)' }}>{currentIdx+1} / {totalChapters}</span>
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'22px', paddingBottom:'18px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
              ...
              <h2 style={{ fontSize:'21px', fontWeight:800, color: 'var(--dax-text-primary)', letterSpacing:'-.02em', margin:0, lineHeight:1.1 }}>{currentChapter?.title}</h2>
            </div>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', gap:'10px', marginTop:'36px', paddingTop:'18px', borderTop:'1px solid rgba(255,255,255,0.06)' }}>
              {prevChapter
                ? <button onClick={() => selectChapter(prevChapter.sectionId, prevChapter.chapterId)}
                    style={{ display:'flex', alignItems:'center', gap:'7px', padding:'10px 14px', background:'var(--dax-surface)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'10px', cursor:'pointer', fontFamily:'inherit', transition:'all .2s', flex:1, maxWidth:'200px' }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor='var(--dax-coral-border)'; (e.currentTarget as HTMLElement).style.background='var(--dax-coral-soft)'; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor='var(--dax-surface-2)'; (e.currentTarget as HTMLElement).style.background='var(--dax-surface)'; }}>
                    ...
                    <div style={{ textAlign:'left' as const, overflow:'hidden' }}>
                      <p style={{ fontSize:'9px', color:'var(--dax-text-muted)', fontWeight:600, letterSpacing:'.08em', textTransform:'uppercase' as const }}>Anterior</p>
                      <p style={{ fontSize:'11px', color:'var(--dax-white-60)', fontWeight:600, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' as const }}>{prevChapter.title}</p>
                    </div>
                  </button>
                : <div/>
              }
              {nextChapter
                ? <button onClick={() => selectChapter(nextChapter.sectionId, nextChapter.chapterId)}
                    style={{ display:'flex', alignItems:'center', gap:'7px', padding:'10px 14px', background:'var(--dax-coral-soft)', border:'1px solid rgba(255,92,53,0.2)', borderRadius:'10px', cursor:'pointer', fontFamily:'inherit', transition:'all .2s', flex:1, maxWidth:'200px', justifyContent:'flex-end' as const }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background='var(--dax-coral-soft)'; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background='var(--dax-coral-soft)'; }}>
            ...
            <p style={{ textAlign:'center' as const, marginTop:'20px', fontSize:'10px', color:'var(--dax-border)' }}>
