...
export function CashExpenseModal({ branchId, accentColor: C, formatCurrency, onClose }: Props) {
  ...
  const { data: shiftData, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/cash-expenses?branchId=${branchId}`).then(r => r.data),
  ...
  const handleSubmit = () => {
    ...
    if (!amt || amt <= 0) return setError('Ingresa un monto valido');
  ...
  const S = {
    label: {
      ...
      textTransform: 'uppercase' as const, color: 'var(--dax-text-muted)',
      marginBottom: '6px', display: 'block',
    },
    input: {
      width: '100%', padding: '10px 13px',
      background: 'var(--dax-surface-2)', border: '1px solid var(--dax-border)',
      borderRadius: '9px', color: 'var(--dax-text-primary)', fontSize: '13px',
      ...
      <div style={{ width: '100%', maxWidth: '520px', background: 'var(--dax-surface)', border: '1px solid var(--dax-border)', borderRadius: '18px', overflow: 'hidden', boxShadow: 'var(--dax-shadow-lg)', animation: 'modalOpen .25s cubic-bezier(.22,1,.36,1)' }}>
        ...
        <div style={{ display: 'flex', gap: '2px', padding: '10px 20px 0', borderBottom: '1px solid var(--dax-border)' }}>
          {([['new', '+ Nuevo gasto'], ['list', `Historial (${expenses.length})`]] as const).map(([t, label]) => (
            <button key={t} onClick={() => setTab(t as 'new' | 'list')}
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    placeholder="0"
                    style={{ ...S.input, paddingLeft: '32px', fontSize: '18px', fontWeight: 800 }}
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    placeholder="Ej: Compra de verduras al proveedor..."
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <select value={category} onChange={e => setCategory(e.target.value)}
                    style={{ ...S.input, paddingLeft: '32px', paddingRight: '28px', appearance: 'none' as any, cursor: 'pointer' }}>
                    {CATEGORIES.map(c => (
                      <option key={c.value} value={c.value} style={{ background: 'var(--dax-surface)' }}>{c.label}</option>
                    ))}
                  </select>
              ...
              <div style={{ padding: '12px 14px', background: 'rgba(240,160,48,0.05)', border: '1px solid rgba(240,160,48,0.15)', borderRadius: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: usePin ? '10px' : 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--dax-text-secondary)' }}>Autorizacion del gerente</span>
                  </div>
                  <div onClick={() => setUsePin(p => !p)}
                    style={{ width: '34px', height: '19px', borderRadius: '10px', background: usePin ? '#F0A030' : 'var(--dax-border)', position: 'relative', cursor: 'pointer', transition: 'background .2s', flexShrink: 0 }}>
                ...
                {!usePin && <p style={{ fontSize: '11px', color: 'var(--dax-text-muted)', marginTop: '2px' }}>Activa para requerir autorizacion</p>}
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {!isLoading && expenses.length === 0 && (
                <div style={{ textAlign: 'center', padding: '40px', color: 'var(--dax-text-muted)' }}>
                  ...
                  <p style={{ fontSize: '12px', marginTop: '4px' }}>Los gastos del turno apareceran aqui</p>
                </div>
              )}
              {expenses.map((exp: any) => {
                ...
                return (
                  <div key={exp.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '12px 14px', background: 'var(--dax-surface-2)', border: '1px solid var(--dax-border)', borderRadius: '11px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '9px', background: `${cat?.color ?? '#6B7280'}20`, border: `1px solid ${cat?.color ?? '#6B7280'}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: cat?.color ?? '#6B7280' }} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' as const }}>
                        <span style={{ fontSize: '10px', fontWeight: 600, color: cat?.color ?? '#6B7280', background: `${cat?.color ?? '#6B7280'}12`, padding: '2px 7px', borderRadius: '20px' }}>{cat?.label ?? exp.category}</span>
                    ...
                    {!exp.authorized && (
                      <button onClick={() => deleteMutation.mutate(exp.id)}
                        style={{ width: '26px', height: '26px', borderRadius: '7px', border: '1px solid rgba(224,80,80,.15)', background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, transition: 'all .15s' }}
